...
exports.postPlannedExercise = async (req, res) => {
  ...
  try {
    ...
    const plannedExercises = formattedWorkoutArr.map((exercise) => {
      const { exerciseType } = exercisesHistory.find(
      ...
      if (exerciseType === "resistance") {
        ...
        if (exerciseHistory && exerciseHistory.exerciseStats.length > 0) {
          latestStats = exerciseHistory.exerciseStats.reduce(
            (latest, stat) => {
    ...
    if (
      ...
      error.message === "No exercise history found."
    ) {
